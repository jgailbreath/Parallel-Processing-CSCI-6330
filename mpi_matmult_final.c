/*  Course:  Parallel Processing Concepts-CSCI-6330
    ...
    Group Members:  J. Gailbreath, R. Morse, E. White. J. Long, D. Shamsieva
...
int main(int argc, char* argv[]){
		...
		//  Main Thread
		if(taskid == MAIN){
			PCHR_FILE = "jAltuve_actual_19X19.csv";  //  File used for pitcher eval.
			BTR_FILE = "mBumgarner_actual_19X19.csv";  //  File used for batter eval.
			csv_to_matrix(SIZE, OG, PCHR_FILE);  //  --can change file pointer to switch between pitcher and batter--
			csv_to_matrix(SIZE, M1, PCHR_FILE);
			numworkers = numtasks - 1;  //  Set workers to be one less than available tasks
